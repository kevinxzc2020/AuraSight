...
import { Colors, Gradients, Spacing, Radius, FontSize, Shadow, StatusColors } from '../../constants/theme'
import { getRecentScans, getStats, ScanRecord, StatsResult } from '../../lib/mongodb'
...
export default function HistoryScreen() {
  ...
  if (loading) {
    return (
      <LinearGradient colors={['#fff5f5', '#ffffff']} style={styles.loadingContainer}>
    ...
    <LinearGradient colors={['#fff5f5', '#ffffff']} style={styles.container}>
      <SafeAreaView style={styles.safeArea}>
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
            ...
            <View style={styles.filterPill}>
              {filters.map((f) => (
                <TouchableOpacity key={f} onPress={() => setActiveFilter(f)} activeOpacity={0.8}>
                  {activeFilter === f ? (
                    <LinearGradient colors={Gradients.roseMain} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.filterActive}>
              ...
              {Array.from({ length: daysInMonth }).map((_, i) => {
                ...
                return (
                  <View key={day} style={styles.dayCell}>
                    <View style={[styles.dayNumWrapper, isToday && styles.todayWrapper]}>
          ...
          <View style={[styles.card, Shadow.card]}>
            <View style={styles.trendHeader}>
              ...
              <View style={styles.trendBadge}>
                ...
                <Text style={[
                  styles.trendValue,
                  { color: (stats?.week_change ?? 0) <= 0 ? Colors.emerald : Colors.red }
                ]}>
            ...
            <View style={styles.sparkline}>
              {sparklineData.map((v, i) => (
                <LinearGradient
                  key={i} colors={Gradients.roseMain}
                  start={{ x: 0, y: 0 }} end={{ x: 0, y: 1 }}
                  style={[styles.sparkBar, { height: (v / maxSparkline) * barHeight }]}
                />
              ))}
            </View>
          </View>
          ...
          {/* Recent Scans */}
          <Text style={[styles.sectionLabel, { marginBottom: Spacing.sm }]}>Recent Scans</Text>

          {recentScans.length === 0 ? (
            <View style={styles.emptyState}>
              <Text style={styles.emptyEmoji}>📷</Text>
              <Text style={styles.emptyText}>No scans yet</Text>
              <Text style={styles.emptySubtext}>Take your first scan to start tracking!</Text>
            </View>
          ) : (
...
const styles = StyleSheet.create({
  ...
  filterPill:  { flexDirection: 'row', backgroundColor: '#fff0f6', borderRadius: Radius.full, padding: 4 },
  filterActive:     { paddingHorizontal: 12, paddingVertical: 4, borderRadius: Radius.full },
  ...
  card:     { backgroundColor: Colors.white, borderRadius: Radius.xxl, padding: Spacing.lg, marginBottom: Spacing.lg },
  monthNav: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: Spacing.md },
  navBtn:   { padding: 4 },
  monthText:{ fontSize: FontSize.base, fontWeight: '600', color: Colors.gray800 },

  weekRow:  { flexDirection: 'row', marginBottom: Spacing.sm },
  weekDay:  { width: cellSize, textAlign: 'center', fontSize: FontSize.xs, color: Colors.gray400, fontWeight: '500' },

  calendarGrid:   { flexDirection: 'row', flexWrap: 'wrap' },
  dayCell:        { width: cellSize, alignItems: 'center', paddingVertical: 4 },
  dayNumWrapper:  { width: 24, height: 24, alignItems: 'center', justifyContent: 'center', borderRadius: 12 },
  ...
  legend:     { flexDirection: 'row', justifyContent: 'center', gap: 20, marginTop: Spacing.md },
  ...
  sectionLabel:{ fontSize: FontSize.sm, fontWeight: '500', color: Colors.gray600 },
  trendBadge:  { flexDirection: 'row', alignItems: 'center', gap: 4 },
  trendValue:  { fontSize: FontSize.sm, fontWeight: '600' },
  sparkline:   { flexDirection: 'row', alignItems: 'flex-end', gap: 3, height: barHeight },
  sparkBar:    { flex: 1, borderTopLeftRadius: 3, borderTopRightRadius: 3, minHeight: 4 },

  recentList:  { gap: Spacing.sm },
  recentCard:  { flexDirection: 'row', alignItems: 'center', gap: Spacing.md, backgroundColor: Colors.white, borderRadius: Radius.xl, padding: Spacing.md },
  recentThumb: { width: 48, height: 48, borderRadius: Radius.lg, alignItems: 'center', justifyContent: 'center' },
  recentInfo:  { flex: 1 },
  recentDate:  { fontSize: FontSize.sm, fontWeight: '500', color: Colors.gray800 },
  ...
  statusBadgeText: { fontSize: FontSize.xs, fontWeight: '500', textTransform: 'capitalize' },

  emptyState:  { alignItems: 'center', paddingVertical: Spacing.xxxl },
  emptyEmoji:  { fontSize: 48, marginBottom: Spacing.md },
  emptyText:   { fontSize: FontSize.base, fontWeight: '600', color: Colors.gray600 },
